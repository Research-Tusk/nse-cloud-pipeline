...
# ═══════════════════════════════════════════════════════════════
# SECTION 7: UPDATE app.js inline data
# ═══════════════════════════════════════════════════════════════

def update_app_js(dashboard_data, enriched_data):
    """Update the inline DATA and ENRICHED_DATA constants in app.js."""
    ...
    data_start = content.index("const DATA = ") + len("const DATA = ")
    enriched_marker = "\n\nconst ENRICHED_DATA = "
    data_end = content.index(enriched_marker)
    enriched_start = data_end + len(enriched_marker)
    enriched_end = content.index("\n\n// ========================", enriched_start)

    new_content = (
        content[:data_start] +
        json.dumps(dashboard_data, separators=(',', ':')) +
        enriched_marker +
        json.dumps(enriched_data, separators=(',', ':')) +
        content[enriched_end:]
    )

    app_js_path.write_text(new_content)
    print(f"Updated {app_js_path} ({len(new_content)} bytes)")
    return True
...
def main():
    ...
    # Step 9: Update app.js with inline data
    update_app_js(dashboard_json, enriched)
